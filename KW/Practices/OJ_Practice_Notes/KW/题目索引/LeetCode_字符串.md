...
> 直接一个个遍历肯定超时  
> 想不出别的办法
### 344. 反转字符串 O(N), c++, 8 ms, 96.48%
### 345. 反转字符串中的元音字母 O(N), c++, 8 ms, 99.22%
* 先将字符串中的元音字母取出
* 再次遍历遇到元音就从取出的元音字母中倒着获取
### 383. 赎金信 O(N+M), c++, 20 ms, 97.32%
* 声明一数组当map用
* 遍历magazine，以magazine[i]当索引++
* 遍历ransomNote，以ransomNote[i]当索引，如果值为0，返回false，不然--
### 387. 字符串中的第一个唯一字符 O(N), c++, 32 ms, 95.25%
* 声明一数组当map用
* 遍历s，以s[i]当索引++
* 遍历s，找到第一个值为1的字符，返回索引